...
\markright{Cross-bridge forces depend on lattice spacing} % Running title

%% ** EDIT HERE **
%% BEGIN MACROS SECTION
\newcommand{\de}{$^\circ$} % Use `$' as \ensuremath kills latex2png
\newcommand{\citep}[1]{\cite{#1}} % Natbib compatibility, 
\newcommand{\citet}[1]{\cite{#1}} % as natbib is not PLoS simpatico
%% END MACROS SECTION

% End of PLoS Formatting
% -------------------------------------------
% Rest of preamble can be commented out without ill effect
% -------------------------------------------



% Bibliography formatting, comment out for PLoS
...
%\usepackage{listings}
%% Allow use of UTF input characters
...
%\title{Axial and Radial Forces of Cross-bridges Depend on Lattice Spacing} 
...
% \maketitle{} % PLoS does this manually
\begin{flushleft}
{\Large
...
Similarly, as lattice spacing increases, decreases in the axial offset of the power stroke rate constant inflection point cause the size of the power stroke to change with lattice spacing (Figs~\ref{fig_kinetics_contours}E-F and S2E-F). %TODO: This would be nice to split, if possible.
